 ...
 * @returns Version as a string.
 */
export function getDescription(toolbox: GluegunToolbox): string {
  const { filesystem } = toolbox

  let directory = toolbox.runtime.defaultPlugin && toolbox.runtime.defaultPlugin.directory
  if (!directory) throw new Error('getDescription: Unknown CLI description (no src folder found)')

  // go at most 5 directories up to find the package.json
  for (let i = 0; i < 5; i += 1) {
    const pkg = filesystem.path(directory, 'package.json')

    // if we find a package.json, we're done -- read the description and return it
    if (filesystem.exists(pkg) === 'file') return filesystem.read(pkg, 'json').description

    // if we reach the git repo or root, we can't determine the description -- this is where we bail
    const git = filesystem.path(directory, '.git')
    const root = filesystem.path('/')
    if (directory === root || filesystem.exists(git) === 'dir') break
  ...
  throw new Error(`getDescription: Unknown CLI description (no package.json found in ${directory}`)
}
...
export function commandInfo(toolbox: GluegunToolbox, commandRoot?: string[]): string[][] {
  return toolbox.runtime.commands
    ...
    .map(command => {
      let aliases = command.aliases.map(alias => {
        return alias.indexOf('-') === -1 ? (alias.length === 1 ? '-' : '--') + alias : '' + alias
      })
      if (command.name !== 'version' && command.name !== 'help') {
        const alias = command.hasAlias() ? `[${aliases.join(', ')}]` : ''
        const commandPath = command.name ? command.commandPath.slice(0, -1).concat(command.name) : command.commandPath

        return [
          toolbox.strings.padEnd(`${commandPath.join(' ')} ${alias}`, HELP_PAD),
 ...
 * @return List of plugins.
 */
export function optionInfo(toolbox: GluegunToolbox, commandRoot?: string[]): string[][] {
  return toolbox.runtime.commands
    .filter(c => !c.hidden)
    .filter(c => !commandRoot || equals(c.commandPath.slice(0, commandRoot.length), commandRoot))
    .map(command => {
      ...
      if (command.name === 'version' || command.name === 'help' || command.type === 'this.$options') {
        let alias = command.hasAlias() ? `${aliases.sort().join(', ')}` : '' // sort as we want long options to appear first
        // const commandPath = command.name ? command.commandPath.slice(0, -1).concat(command.name) : command.commandPath
        if (!command.description && command.name === 'help') {
          command.description = 'Shows help (this screen)'
        }
        return [
          toolbox.strings.padEnd(` ${alias}`, HELP_PAD),
          replace('$BRAND', toolbox.runtime.brand, command.description || '-'),
        ]
      } else {
        return []
